...
class Receptionist extends Controller
{
    ...
    function generateUniqueId($prefix = 'MB/F/') {
                ...
                case 'registerMember':
                    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                        ...
                        if ($member->validate($_POST) && $user->validate($_POST)) {
                            ...
                            do {
                                $offset = str_pad(mt_rand(1, 9999), 4, '0', STR_PAD_LEFT);
                                $temp['member_id'] = substr($temp['member_id'], 0, 5) . $offset;
                                $temp['user_id'] = $temp['member_id'];
                            } while ($member->findByMemberId($temp['member_id']));
                    
                            $temp['password'] = password_hash($temp['password'], PASSWORD_DEFAULT);
                            $temp['status'] = 'Active';
                    
                            // Validate membership plan
                            if (empty($temp['membership_plan'])) {
                                $data['errors']['membership_plan'] = 'Membership plan is required.';
                            ...
                            try {
                                ...
                                if ($e->getCode() == 23000) {
                                    $data['errors']['database'] = 'A member with this ID already exists. Please try again.';
                                } else {
                                    $data['errors']['database'] = 'An error occurred while registering the member.';
                                }
                                $this->view('receptionist/receptionist-createMember', $data);
                            }
                        } else {
